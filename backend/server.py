...
async def subscribe_to_kit(
    ...
    # Verify all assessment fields are populated
    if not risk_level:
        logger.warning("WARNING: risk_level is empty!")
    if not score:
        logger.warning("WARNING: score is empty!")
    if not top_risks:
        logger.warning("WARNING: top_risks is empty!")
